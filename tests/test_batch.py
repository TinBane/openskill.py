"""
All tests for the batch processing module are located here.
"""
...
def test_independent_games_single_wave() -> None:
    """
...
def test_dependent_games_separate_waves() -> None:
    """
...
def test_no_entity_appears_twice_in_wave() -> None:
    """
...
def test_preserves_all_games() -> None:
    """
...
def test_all_games_share_entity() -> None:
    """
    Worst case: all games share one entity produces one game per wave.
    """
    games = [
...
def test_ordering_preserved_across_waves() -> None:
    """
...
# Wave Ordering Invariant


def test_no_leapfrogging() -> None:
    """
    Game i before game j (shared entities) must have wave(i) < wave(j).
    """
    games = [
        Game(teams=[["a", "b"], ["c", "d"]]),
        Game(teams=[["a", "c"], ["e", "f"]]),
...
def test_power_law_no_violations() -> None:
    """
    Stress test with power-law player distribution.
    """
...
def test_wave_scan_skips_occupied_waves() -> None:
    """
    ...
    waves = partition_waves(games)
    assert len(waves) == 3
    wave_indices = {idx: w_idx for w_idx, wave in enumerate(waves) for idx, _ in wave}
...
def test_sequential_matches_manual_rate() -> None:
    """
    Sequential batch processing must match manual :code:`rate()` calls.
    """
    model = PlackettLuce()
...
def test_pipelined_matches_sequential() -> None:
    """
...
def test_initial_ratings_respected() -> None:
    """
...
def test_batch_empty_games() -> None:
    """
...
def test_many_independent_games() -> None:
    """
...
def test_chain_of_dependent_games() -> None:
    """
    Chain where each game depends on the previous (worst case).
    """
    model = PlackettLuce()

    games = [Game(teams=[["p0"], [f"opponent_{i}"]], ranks=[1, 2]) for i in range(10)]
...
def test_multiprocess_matches_sequential() -> None:
    """
...
def test_weights_passed_through() -> None:
    """
...
def test_limit_sigma() -> None:
    """
    Ensures :code:`limit_sigma=True` prevents sigma from increasing.
    """
...
def test_no_ranks_no_scores() -> None:
    """
...
def test_multiprocess_path_pipelined() -> None:
    """
    Force :code:`ProcessPoolExecutor` path in pipelined mode.
    """
    model = PlackettLuce()
...
def test_multiprocess_path_non_pipelined() -> None:
    """
    Force :code:`ProcessPoolExecutor` path in non-pipelined mode.
    """
    model = PlackettLuce()
...
def test_multiprocess_large_wave() -> None:
    """
    Force :code:`ProcessPoolExecutor` with waves having more than 2 games.
    """
    model = PlackettLuce()

    games = [Game(teams=[[f"p{i*2}"], [f"p{i*2+1}"]], ranks=[1, 2]) for i in range(10)]
...
def test_multiprocess_scores_and_weights() -> None:
    """
    :code:`ProcessPoolExecutor` path with scores and weights.
    """
    model = PlackettLuce()
...
def test_multiprocess_limit_sigma() -> None:
    """
    :code:`ProcessPoolExecutor` path respects :code:`limit_sigma`.
    """
    model = PlackettLuce(limit_sigma=True)

    games = [Game(teams=[[f"p{i*2}"], [f"p{i*2+1}"]], ranks=[1, 2]) for i in range(5)]
...
def test_initial_ratings_unknown_entity_ignored() -> None:
    """
    Ensures :code:`initial_ratings` with entity not in any game is
    silently skipped.
    """
    model = PlackettLuce()
    processor = BatchProcessor(model, n_workers=1)
    ratings = processor.process(
        [Game(teams=[["a"], ["b"]], ranks=[1, 2])],
        initial_ratings={"a": (30.0, 5.0), "unknown": (40.0, 2.0)},
    )
    assert "unknown" not in ratings
    assert "a" in ratings
...
def test_fast_rating_basic() -> None:
    """
    Ensures default :code:`_FastRating` parameters work correctly.
    """
...
def test_fast_rating_ordinal_default() -> None:
    """
    Ensures the default ordinal calculation works correctly.
    """
    r = _FastRating(25.0, 8.333)
    assert r.ordinal() == pytest.approx(25.0 - 3.0 * 8.333)


def test_fast_rating_ordinal_custom_params() -> None:
    """
    Ensures custom ordinal parameters work correctly.
    """
    r = _FastRating(25.0, 8.333)
...
def test_extract_model_config_basic() -> None:
    """
    Ensures basic model config extraction works.
    """
    model = PlackettLuce()
...
def test_extract_model_config_with_margin_balance() -> None:
    """
    Ensures :code:`ThurstoneMostellerFull` config includes margin,
    balance, and weight_bounds.
    """
...
def test_extract_model_config_with_weight_bounds() -> None:
    """
    Ensures :code:`BradleyTerryFull` config includes weight_bounds.
    """
...
def test_extract_model_config_no_optional_attrs() -> None:
    """
    Ensures model missing optional attrs doesn't include them in kwargs.
    """
    model = PlackettLuce()

    # Temporarily remove optional attributes
    saved = {}
    for attr in ("gamma", "margin", "balance", "weight_bounds"):
    ...
    try:
        ...
        assert "weight_bounds" not in kwargs
    finally:
...
def test_init_worker() -> None:
    """
    Ensures :code:`_init_worker` initializes global model state.
    """
    model = PlackettLuce()
    module, class_name, kwargs = _extract_model_config(model)
...
def test_worker_rate_game() -> None:
    """
    Ensures :code:`_worker_rate_game` produces correct results.
    """
    model = PlackettLuce()
    module, class_name, kwargs = _extract_model_config(model)
    _init_worker(module, class_name, kwargs)
...
def test_worker_rate_game_with_scores() -> None:
    """
    Ensures :code:`_worker_rate_game` works with scores instead of ranks.
    """
    model = PlackettLuce()
    module, class_name, kwargs = _extract_model_config(model)
    _init_worker(module, class_name, kwargs)
...
def test_worker_rate_game_with_weights() -> None:
    """
    Ensures :code:`_worker_rate_game` works with weights.
    """
    model = PlackettLuce()
    module, class_name, kwargs = _extract_model_config(model)
    _init_worker(module, class_name, kwargs)
...
def test_worker_rate_game_limit_sigma() -> None:
    """
    Ensures :code:`_worker_rate_game` respects :code:`limit_sigma`.
    """
    model = PlackettLuce(limit_sigma=True)
    module, class_name, kwargs = _extract_model_config(model)
    _init_worker(module, class_name, kwargs)
...
def test_worker_rate_game_no_ranks_no_scores() -> None:
    """
    Ensures :code:`_worker_rate_game` works with neither ranks nor scores.
    """
    model = PlackettLuce()
    module, class_name, kwargs = _extract_model_config(model)
    _init_worker(module, class_name, kwargs)
